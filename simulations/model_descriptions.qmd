...
reltime_0.95_CI <- list(NA, NA, NA, NA, NA, NA,
          c(-0.05, 0.15), c(0.23, 0.45), c(0.28, 0.41), c(0.38, 0.6), c(0.78, 0.93))
...
## Part A
```{r}
partA_left <-  ggtree(tree, ladderize = FALSE)  %<+% d + 
  ...
  geom_richtext(label = "Variance in<br>divergence time",
                x = 0.05, y = 2.08,
                label.color = NA) +
  geom_richtext(label = "Ancestral state<br>reconstruction",
                x = 0, y = 6.6,
                label.color = NA) +
...
partB_left <- static_tree %>% insert_right(points_plot, width = 0.8)
```
...
```{r}
partB <- partB_left + arrowplot + partB_right + 
  plot_annotation(
